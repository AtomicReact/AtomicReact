...
export class Atomic {
    constructor(config, hotReload) {
        ...
        if (!this.config.atomicDir) {
            console.log(ConsoleFlags.erro, "You must set an atomicDir where all yours atoms will be");
            return;
        }
    ...
    static readAtomsDir(dirPath, callback, extensions = [".html"], atomKey) {
    ...
    async bundle() {
        if (this.config.debug) {
            console.log(ConsoleFlags.info, "===Bundling===");
        }
        ...
        Atomic.readAtomsDir(this.config.atomicDir, (atomKey, filePath) => {
            ...
            if (this.config.debug)
                console.log(`\t[LOGIC] [${atomKey}] => ${filePath}`);
        ...
        Atomic.readAtomsDir(this.config.atomicDir, (atomKey, filePath) => {
            if (this.config.debug)
                console.log(`\t[STYLE] [${atomKey}] => ${filePath}`);
